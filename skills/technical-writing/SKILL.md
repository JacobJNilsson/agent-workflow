...
description: Use for /technical-writing or when writing or reviewing docs, RFCs, readmes, specs, code comments, PR descriptions, or commit messages. The standard that copy-review, commit-message, and pr-description point at.
---
...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. When you meet a new offender, add it and its replacement to rule 26 of the `unslop` skill.

The `unslop` skill owns the catalog of AI tells: vocabulary, filler, hedging, and formatting. Apply it to every text this skill touches.
...
## Make statements load one at a time (ASD-STE100 rules)

- One instruction per sentence. One thought per sentence everywhere else.
- Split instructions longer than about 20 words and other sentences longer than about 25. Count, do not guess.
...
- Use periods, not semicolons. Replace an em dash with a new sentence or a comma.
...
- Call each thing by one name, everywhere. A doc that says "the gate", "the ratchet", and "the budget check" for one thing teaches three things. This holds across code, comments, and tests too. When two words compete for one meaning, count them, keep the one in wider use, and replace the rest. Rewording an unchanged sentence between edits costs the same way. Don't churn what didn't change.
...
**Code comments.** A comment states a fact the code cannot show. It does not narrate what the next line does. One sentence is the norm, two is the limit. Most functions need no comment: the name and body say what, a comment survives only for a why. The register is terse: "Route footprint in kg CO2e per transported kg. Stamped with calculation time and change ids." Drop articles and lead-ins where the sentence still reads one way. This holds for SQL and type comments too. Say the state, not the mechanism that produced it: "a route without a footprint", not "a route no recalculation has reached". Never write a comment that answers a review. That reason belongs in the PR thread.

**Commit messages and PR descriptions.** Every layer except Diátaxis applies. The `commit-message` and `pr-description` skills hold the shape and the audience test. A PR body is a briefing that a reviewer reads in under a minute. Do not paste logs, SHA lists, or metric tables. Link them.

**Spec text.** The spec repo's own rules apply on top: present tense, behaviour stated as fact, open questions marked open, terms from the glossary.

**Product UI strings and website copy.** Not documentation. Use the product's copy guidelines, or the `website-copy-review` skill for a site.

**Code in prose.** Follow the repo's formatter for snippets. Write real paths and real symbols. Make every count or tree claim true at the commit that lands it, and include the command that regenerates it.
...
Source: adapted from the pstack `technical-writing` skill (github.com/cursor/plugins, MIT).
