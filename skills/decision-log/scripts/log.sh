#!/usr/bin/env bash
# Append one row to a decision log (TSV).
...
# Keep each cell on one line. Prefix a cell that starts with =, +, - or @
# with a quote, so a spreadsheet does not read it as a formula.
clean() {
	local v
	v=$(printf '%s' "$1" | tr '\t\n\r' '   ')
	case "$v" in
